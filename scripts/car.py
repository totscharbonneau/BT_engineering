...
for i in range(0, 4):
    ...
    bpy.ops.mesh.primitive_cylinder_add(
        radius=wheel_radius,
        depth=2.8,
        location=((-1)**i*(picar_width/2 + wheel_width/2 + 0.1), back_wheel_distance, wheel_radius),
        scale=(1, 1, 1),
...
bpy.ops.mesh.primitive_cube_add(
    size=1,
    location=(0, picar_length/2+line_sensor_length/2, line_sensor_elevation),
...
for i in range(0, 5):
    bpy.ops.mesh.primitive_cylinder_add(
        radius=0.2,
        depth= sensor_height,
        location=((i-2)*sensors_distances, picar_length/2+ 3/4*line_sensor_length, line_sensor_elevation-sensor_height/2 ),
        rotation=(0, 0, 0)
    )
...
bpy.ops.mesh.primitive_cube_add(
    size=1,
    location=(0, picar_length/2, picar_height+2),
    scale=(2, 0.5, 1)
)